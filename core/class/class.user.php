<?php
	...
	class User extends DB_connect {
		...
		public function selectAdminUser ( $id = NULL )
		{
			if ( $id == NULL OR $id == 0 )
			{
				$query = "select x.rgt_id, x.salontype_id, concat ( x.salutation, ' ', x.fname, ' ', x.mname, ' ', x.lname ) as name, x.eml_login, x.honour, x.club, concat ( trim( x.addr1 ), '\n', trim( x.addr2 ), '\n', trim( x.addr3 ), '\n', trim( x.city_town_vill ), '\n', trim( x.state_province ), '\n', trim( x.zipcode ), '\n', trim( y.country )) as address, x.mobile,z.payment_due from registrant x inner join countries y on x.country = y.country_id left join payment_stat z on x.rgt_id=z.reg_id";
			...
			else
			{
				...
				*/
				...
				$query = "select x.rgt_id, x.salontype_id, 
							concat ( x.salutation, ' ', x.fname, ' ', x.mname, ' ', x.lname ) as name, 
							x.eml_login, x.honour, x.club, 
							concat ( trim( x.addr1 ), '\n', trim( x.addr2 ), '\n', trim( x.addr3 ), '\n', 
							trim( x.city_town_vill ), '\n', trim( x.state_province ), '\n', 
							trim( x.zipcode ), '\n', trim( y.country )) as address, 
							x.mobile,z.payment_due 
							from registrant x 
							inner join countries y on x.country = y.country_id 
							left join payment_stat z on x.rgt_id=z.reg_id
							where x.rgt_id = '".$id."'";
			}
			...
			try 
			{
				...
				$stmt -> prepare ( $query );
				$stmt -> execute ();
				$stmt -> bind_result ( $rgt_id, $salontype_id, $name, $eml_login, $honour, $club, $address, $mobile, $payment_due );
				$user_dtl = [];
				while ( $stmt -> fetch () )
				{
