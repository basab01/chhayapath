<?php
	...
	if ( isset ( $_GET['token'] ))
	{
		if ( isset ( $_SESSION['admin'] ) AND ( $_GET['token'] == $_SESSION['admintoken'] ) )
		{
			...
			$prvpage = 'adminuserunlock.php?type='.$type.'&token='.$ttok.'&usrid='.$userid;
			//echo $page;
			...
			if ( $_POST )
			{
				...
				else
				{
					...
					{
					$ss = $st->updateLockStatus($_GET['usrid'], $_POST);
					//echo 'value : '.$ss.'<br />';
					if ( $ss == 1 ) $msg = '<span style="color:green;">Update Successful !!</span> <a class="btn btn-primary" href="'.$prvpage.'">Back</a>';
					}
					else
					{
						$msg = '<span style="color:red;">Update Unsuccessful!!</span> <a href="'.$page.'">Back</a>';
					}
				}
			...
			include_once "assets/templates/adminUnlockUpdateTemplate.php";
		}
		else
		{
			$flag = 1;
